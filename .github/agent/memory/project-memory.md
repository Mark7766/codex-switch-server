...
**首页**：Hero → 价值主张（三列卡片）→ 支持工具展示（图标网格）→ 用户故事 → 底部 CTA → 页脚。10 秒黄金法则：用户扫一眼就知道这是什么、能干什么、怎么下载。

**下载页**：平台切换（分段控件）→ 最新版本大卡片（版本号 + 日期 + 文件大小 + CTA 按钮）→ 系统要求 → 历史版本（details/summary 可折叠）。

**使用指南**：三步向导交互（选工具 → 选平台 → 线性 6 步安装指南）：获取 API Key → 下载工具（按平台动态匹配）→ 安装工具 → 下载 Codex Switch（macOS 需 xattr 命令）→ 配置 API Key + 启动代理 → 启动验证。10 张截图（`src/static/images/guide/`）按场景分组加载，`onload` 自动显示。
